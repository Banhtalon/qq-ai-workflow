...
async function atomicJson(file,value){await mkdir(path.dirname(file),{recursive:true});const tmp=file+'.'+randomUUID()+'.tmp';const fd=await open(tmp,'wx');try{await fd.writeFile(JSON.stringify(value,null,2)+'\n');await fd.sync();}finally{await fd.close();}await rename(tmp,file);}
...
async function appendChain(root,receipt){const chain=await nextChain(root);receipt.prev_receipt_sha256=chain.last_receipt_sha256;receipt.receipt_sha256=receiptHash(receipt);await atomicJson(path.join(root,'.receipts-chain.json'),{schema_version:RECEIPT_SCHEMA_VERSION,last_receipt_sha256:receipt.receipt_sha256});return receipt;}
export async function beginInvocation({packetDir,role,binding,prompt,started_at}){
  const kind=kindFor(role);if(!kind)throw Error('invalid receipt role');const context=await contextFor(packetDir);const {state,task}=context;const run_id=state?.run_id??null,task_id=task?.task_id??null,assignment_id=randomUUID();
  const assignment={schema_version:RECEIPT_SCHEMA_VERSION,receipt_type:'ASSIGNMENT',assignment_id,task_id,run_id,kind,phase:role,role,provider:binding.provider,cli:binding.cli??null,requested_model:binding.model,requested_effort:binding.effort??null,
...
export async function finishInvocation({packetDir,role,binding,prompt,result,started_at,finished_at,context}){
  const kind=kindFor(role);if(!kind)throw Error('invalid receipt role');const {state,task,root}=context??await contextFor(packetDir);const assignment_id=context?.assignment?.assignment_id??null;
  const execution={schema_version:RECEIPT_SCHEMA_VERSION,receipt_type:'EXECUTION',receipt_id:randomUUID(),task_id:task?.task_id??null,run_id:state?.run_id??null,assignment_id,kind,phase:role,role,provider:binding.provider,cli:binding.cli??null,requested_model:binding.model,observed_models:result?.observed_models??[],requested_effort:binding.effort??null,
  ...
  const chained=await appendChain(root,execution);await atomicJson(path.join(packetDir,'receipts','execution.json'),chained);return chained;
}
