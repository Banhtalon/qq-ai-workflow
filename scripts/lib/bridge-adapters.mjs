...
export function validateBinding(b) {
  ...
  // An executable, or node plus a JS entry point; no user-supplied flags that
...
export async function doctor(binding,{cwd,packetDir,probe=false,signal}={}) {
  ...
  const version=await execute([...binding.command,'--version'],{cwd,timeoutSeconds:20,signal});
  ...
  if(binding.provider==='openai'){
    ...
    if(auth.code!==0||! /Logged in using ChatGPT/i.test(auth.stdout+auth.stderr))return {...report,status:'WAITING_CAPABILITY',auth:'CHATGPT_LOGIN_REQUIRED'};
    report.auth='CHATGPT';
  }
