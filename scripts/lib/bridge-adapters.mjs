...
export function validateBinding(b) {
  if(!b||!['openai','google'].includes(b.provider)||!Array.isArray(b.command)||!b.command.length||
    b.command.some(x=>typeof x!=='string'||!x)||!/^[-a-zA-Z0-9_.]+$/.test(b.model??''))throw Error('invalid CLI binding');
...
export async function invocation(binding,{cwd,packetDir,role,prompt}) {
  ...
  if(b.provider==='openai'){
    ...
    const args=[...b.command,'exec','--ignore-user-config','-c','forced_login_method="chatgpt"','-c','model_provider="openai"',
      '-c','approval_policy="never"','-m',b.model,'-s',role==='worker'?'workspace-write':'read-only','--json','--output-schema',schema,'-C',cwd];
...
export function parseProtocol(provider,stdout,cli='gemini',{capabilityProbe=false}={}) {
  ...
  if(provider==='openai') {
    const events=stdout.trim().split(/\r?\n/).map(line=>JSON.parse(line));const starts=events.filter(e=>e.type==='thread.started');
    ...
    session=starts[0].thread_id;const messages=events.filter(e=>e.type==='item.completed'&&e.item?.type==='agent_message');body=messages.at(-1)?.item.text;
    models=[...new Set(events.flatMap(e=>e.model?[e.model]:[]))];usage=events.filter(e=>e.type==='turn.completed').at(-1)?.usage??null;
  } else if(cli==='antigravity') {
    const events=stdout.trim().split(/\r?\n/).map(line=>JSON.parse(line));const results=events.filter(e=>e.event==='result');
    if(results.length!==1||results[0].result?.status!=='SUCCESS')throw Error('incomplete Antigravity protocol');const response=results[0].result;session=response.conversation_id;
    usage=response.usage??response.stats??null;models=[...new Set(events.flatMap(e=>e.init?.model?[e.init.model]:[]))];body=response.structured_output?JSON.stringify(response.structured_output):response.response;
  } else {
...
export async function invoke(binding,options) {
  ...
  const record=safe({provider:binding.provider,requested_model:binding.model,requested_effort:binding.effort??null,argv:spec.argv,code:r.code,reason:r.reason,
    started_at:r.started_at??started_at,finished_at:r.finished_at??new Date().toISOString(),status:failureStatus(r)});
  ...
  await finishInvocation({packetDir:options.packetDir,role:options.role,binding,prompt:options.prompt,result:record,started_at:record.started_at,finished_at:record.finished_at,context});
  return record;
}
export async function doctor(binding,{cwd,packetDir,probe=false,signal}={}) {
  ...
  if(binding.provider==='openai'){
    const auth=await execute([...binding.command,'login','status'],{cwd,timeoutSeconds:20,signal});
    if(auth.code!==0||! /Logged in using ChatGPT/i.test(auth.stdout+auth.stderr))return {...report,status:'WAITING_CAPABILITY',auth:'CHATGPT_LOGIN_REQUIRED'};report.auth='CHATGPT';
  }
  if(!probe)return report;let result;
  try{result=await invoke(binding,{cwd,packetDir,role:'probe',timeoutSeconds:90,signal,prompt:'Capability probe. Do not use tools or change files. Reply only with JSON: {"verdict":"PASS","summary":"subscription CLI probe","material_findings":[],"risk_checks_completed":false}'});}
