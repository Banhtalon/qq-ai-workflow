...
export function parseProtocol(provider,stdout,cli='gemini',{capabilityProbe=false}={}) {
  ...
  if(provider==='openai') {const events=stdout.trim().split(/\r?\n/).map(line=>JSON.parse(line));const starts=events.filter(e=>e.type==='thread.started');const completed=events.findLastIndex(e=>e.type==='turn.completed'),lastTransient=events.findLastIndex(transientOpenAITransportEvent);if(starts.length!==1||completed<0||lastTransient>=completed||events.some(e=>e.type==='turn.failed'||(e.type==='error'&&!transientOpenAITransportEvent(e))))throw Error('incomplete Codex protocol');session=starts[0].thread_id;const messages=events.filter(e=>e.type==='item.completed'&&e.item?.type==='agent_message');body=messages.at(-1)?.item.text;models=[...new Set(events.flatMap(e=>e.model?[e.model]:[]))];usage=events.filter(e=>e.type==='turn.completed').at(-1)?.usage??null;
  } else if(cli==='antigravity') {const events=stdout.trim().split(/\r?\n/).map(line=>JSON.parse(line));const results=events.filter(e=>e.event==='result');if(results.length!==1||results[0].result?.status!=='SUCCESS')throw Error('incomplete Antigravity protocol');const response=results[0].result;session=response.conversation_id;usage=response.usage??response.stats??null;models=[...new Set(events.flatMap(e=>e.init?.model?[e.init.model]:[]))];body=response.structured_output?JSON.stringify(response.structured_output):response.response;
  } else {const response=JSON.parse(stdout);if(response.error)throw Error('Gemini protocol error');session=response.session_id;models=Object.keys(response.stats?.models??{});body=response.response;usage=response.usage??response.stats??null;}
  if(typeof session!=='string'||!session.trim()||typeof body!=='string')throw Error('missing session or structured response');const result=JSON.parse(body.replace(/^```(?:json)?\s*\n?/,'').replace(/\n?```\s*$/,''));if(result&&Object.keys(result).some(k=>!['verdict','summary','material_findings','risk_checks_completed'].includes(k)))throw Error('unexpected structured result field');if(!result||!['PASS','NEEDS_FIX','BLOCKED'].includes(result.verdict)||typeof result.summary!=='string'||!Array.isArray(result.material_findings)||!result.material_findings.every(x=>typeof x==='string')||typeof result.risk_checks_completed!=='boolean'||(result.verdict==='PASS'&&result.material_findings.length&&!capabilityProbe))throw Error('invalid structured result');return safe({session_id:session,observed_models:models,usage,result});
}
